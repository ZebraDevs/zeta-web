...
import { write, log, replaceAllInTemplate, capitalize, convertPathToUnixStyle, pascalCaseFromKebab } from './utils.js';
 ...
 */
const getWebStories = () => walker("./src").filter(x => x.match(/\.stories\.(ts|js|tsx|jsx)$/));
 ...
 */
const pathToName = (path) => convertPathToUnixStyle(path).split('/').pop().split('.').shift();
...
const outputStoryFile = (storyPath) => {
  ...
  let storyWeb = storyText.split(/$/).map(x => x.includes('render') ? '//TODO: This render method may need to change for react\n' + x : x).join('\n');
  ...
  populatedTemplate = replaceAllInTemplate(`"zeta-${name}"`, `Zeta${camelCaseName}`, populatedTemplate);
  populatedTemplate = replaceAllInTemplate(`"Zeta${camelCaseName}"`, `Zeta${camelCaseName}`, populatedTemplate);
  populatedTemplate = replaceAllInTemplate(`import { Zeta${camelCaseName} } from`, `import Zeta${camelCaseName} from`, populatedTemplate);
  populatedTemplate = replaceAllInTemplate(`import { ifDefined } from "lit/directives/if-defined.js";`, '', populatedTemplate); 
  populatedTemplate = replaceAllInTemplate(`import { spread } from "@open-wc/lit-helpers";`, '', populatedTemplate);
  
  
  populatedTemplate = populatedTemplate.replace(/import \{ Zeta([A-Z][a-zA-Z]+) \} from "..\//, "import Zeta$1 from \"../", populatedTemplate);
  populatedTemplate = populatedTemplate.replace(/from "((?:..\/)+)(?:.+\/)?([a-zA-Z-]+).js"/g, `from "\$1src/\$2/index.js"`, populatedTemplate); //Path of react index file
  populatedTemplate = populatedTemplate.replace(/import "((?:..\/)+)(?:.+\/)?([a-zA-Z-]+).js"/g, (_, a, b) => `import Zeta${pascalCaseFromKebab(b)} from "${a}src/${b}/index.js"`, populatedTemplate); //Path of other react components used in this story index file
  populatedTemplate = populatedTemplate.replace(/ifDefined\(([a-zA-Z\._-]+)\)/g, '\$1', populatedTemplate) //removes ifDefined
  populatedTemplate = populatedTemplate.replace(/\$\{spread\(args\)\}/g, '{...args}', populatedTemplate) //converts the usage of spread() function to actual ...spread in React
  populatedTemplate = populatedTemplate.replace(/\$\{args\.([A-Za-z0-9._-]+)\}/g, '{args.\$1}', populatedTemplate) //converts ${...} to {...}
  populatedTemplate = populatedTemplate.replace(/\$\{/g, '{', populatedTemplate) //converts remaining ${ to { . Probably dont need the line above


  /** Refactor local imports to come from react index file */
  let localImports = [];
  const populatedTemplateArr = populatedTemplate.split(/$/).map(line => {
...
populatedTemplate = populatedTemplate.split(/html`\s*/s).map((y, index) => {
    ...
    return y.replace(/<(\/?)zeta-([a-z0-9-]+)(\s|>|\/>)/g, (match, group1, group2, group3, endTick) => {
      return `<${group1}Zeta${capitalize(group2)}${group3}`;
    }).replace(/(\s+)[\.@\?]([a-zA-Z0-9_-]+)=/g, "$1$2=").replace(/<\/([a-zA-Z-]+)\s*>\s*`/gs, '</$1>');
  }).join('')
...
const main = () => {
  ...
  const currentReactStoriesConv = walker(SRC_PATH).filter(x => x.includes('.stories.')).map(x => x.replace('/zeta-react/', '/').slice(0, -1));
  ...
  if (allStoriesNeeded.length > 0) {
    ...
    log(`✅ --- All ${allStoriesNeeded.length} stories un-reactified to ${path.resolve(ZETA_FOLDER_PATH)}`)
  } else {
