...
describe("zeta-table additional coverage", () => {
  ...
  describe("Select on Row Click", () => {
    it("toggles selection when selectOnRowClick is true", async () => {
      ...
      assert.include(ev.detail.selectedIds, 1);
    });
    ...
    it("deselects on second click", async () => {
      ...
      setTimeout(() => row.click());
      const ev = await oneEvent(el, "selectionChange");
      assert.notInclude(ev.detail.selectedIds, 1);
    });
  });
  ...
  describe("Export", () => {
    it("dispatches tableExport event with CSV data", async () => {
      ...
      assert.isString(ev.detail.csv);
      assert.isArray(ev.detail.columns);
      assert.isArray(ev.detail.data);
      assert.include(ev.detail.csv, "Name");
      assert.include(ev.detail.csv, "Alice");
    });
  });
  ...
  describe("Column Configure Reorder", () => {
