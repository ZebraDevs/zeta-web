...
export const Form: StoryObj = {
  render: () => html`
    ...
    >
      <fieldset .disabled=${disabled}>
        <div style=${columnStyle}>
          ...
          <div style=${rowStyle}>
            <div style=${columnStyle}>
              <zeta-text-input id="zeta-input" name="zeta-input" placeholder="Placeholder" hintText="Hint text" value="Default value"
                >Text Input</zeta-text-input
              >
              <zeta-text-input id="zeta-date-input" name="zeta-date-input" placeholder="Placeholder" hintText="Hint text" type="date"
                >Date Input</zeta-text-input
              >
              <zeta-text-input id="zeta-time-input" name="zeta-time-input" placeholder="Placeholder" hintText="Hint text" type="time"
                >Time Input</zeta-text-input
              >
            </div>
            <div style=${columnStyle}>
        ...
        <div style=${rowStyle}>
          ...
          <fieldset
            style=${styleMap({ display: "flex", flexDirection: "column", gap: "var(--spacing-small)", flex: 1, margin: 0, borderWidth: 0, padding: 0 })}
          >
    ...
    <zeta-button
      @click=${() => {
        document.querySelectorAll("fieldset").forEach(fs => (fs.disabled = !fs.disabled));
      }}
      >test</zeta-button
    >
  `
