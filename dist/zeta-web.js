import { css as M, LitElement as rt, html as v, svg as Dt, nothing as tt } from "lit";
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
const k = (n) => (t, e) => {
  e !== void 0 ? e.addInitializer(() => {
    customElements.define(n, t);
  }) : customElements.define(n, t);
};
/**
 * @license
 * Copyright 2019 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
const Z = globalThis, st = Z.ShadowRoot && (Z.ShadyCSS === void 0 || Z.ShadyCSS.nativeShadow) && "adoptedStyleSheets" in Document.prototype && "replace" in CSSStyleSheet.prototype, yt = Symbol(), lt = /* @__PURE__ */ new WeakMap();
let Ct = class {
  constructor(t, e, r) {
    if (this._$cssResult$ = !0, r !== yt)
      throw Error("CSSResult is not constructable. Use `unsafeCSS` or `css` instead.");
    this.cssText = t, this.t = e;
  }
  get styleSheet() {
    let t = this.o;
    const e = this.t;
    if (st && t === void 0) {
      const r = e !== void 0 && e.length === 1;
      r && (t = lt.get(e)), t === void 0 && ((this.o = t = new CSSStyleSheet()).replaceSync(this.cssText), r && lt.set(e, t));
    }
    return t;
  }
  toString() {
    return this.cssText;
  }
};
const Et = (n) => new Ct(typeof n == "string" ? n : n + "", void 0, yt), Pt = (n, t) => {
  if (st)
    n.adoptedStyleSheets = t.map((e) => e instanceof CSSStyleSheet ? e : e.styleSheet);
  else
    for (const e of t) {
      const r = document.createElement("style"), s = Z.litNonce;
      s !== void 0 && r.setAttribute("nonce", s), r.textContent = e.cssText, n.appendChild(r);
    }
}, ct = st ? (n) => n : (n) => n instanceof CSSStyleSheet ? ((t) => {
  let e = "";
  for (const r of t.cssRules)
    e += r.cssText;
  return Et(e);
})(n) : n;
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
const { is: It, defineProperty: kt, getOwnPropertyDescriptor: Ot, getOwnPropertyNames: Ft, getOwnPropertySymbols: Lt, getPrototypeOf: Rt } = Object, b = globalThis, dt = b.trustedTypes, Ht = dt ? dt.emptyScript : "", z = b.reactiveElementPolyfillSupport, H = (n, t) => n, J = { toAttribute(n, t) {
  switch (t) {
    case Boolean:
      n = n ? Ht : null;
      break;
    case Object:
    case Array:
      n = n == null ? n : JSON.stringify(n);
  }
  return n;
}, fromAttribute(n, t) {
  let e = n;
  switch (t) {
    case Boolean:
      e = n !== null;
      break;
    case Number:
      e = n === null ? null : Number(n);
      break;
    case Object:
    case Array:
      try {
        e = JSON.parse(n);
      } catch {
        e = null;
      }
  }
  return e;
} }, nt = (n, t) => !It(n, t), ht = { attribute: !0, type: String, converter: J, reflect: !1, hasChanged: nt };
Symbol.metadata ?? (Symbol.metadata = Symbol("metadata")), b.litPropertyMetadata ?? (b.litPropertyMetadata = /* @__PURE__ */ new WeakMap());
class L extends HTMLElement {
  static addInitializer(t) {
    this._$Ei(), (this.l ?? (this.l = [])).push(t);
  }
  static get observedAttributes() {
    return this.finalize(), this._$Eh && [...this._$Eh.keys()];
  }
  static createProperty(t, e = ht) {
    if (e.state && (e.attribute = !1), this._$Ei(), this.elementProperties.set(t, e), !e.noAccessor) {
      const r = Symbol(), s = this.getPropertyDescriptor(t, r, e);
      s !== void 0 && kt(this.prototype, t, s);
    }
  }
  static getPropertyDescriptor(t, e, r) {
    const { get: s, set: i } = Ot(this.prototype, t) ?? { get() {
      return this[e];
    }, set(o) {
      this[e] = o;
    } };
    return { get() {
      return s == null ? void 0 : s.call(this);
    }, set(o) {
      const l = s == null ? void 0 : s.call(this);
      i.call(this, o), this.requestUpdate(t, l, r);
    }, configurable: !0, enumerable: !0 };
  }
  static getPropertyOptions(t) {
    return this.elementProperties.get(t) ?? ht;
  }
  static _$Ei() {
    if (this.hasOwnProperty(H("elementProperties")))
      return;
    const t = Rt(this);
    t.finalize(), t.l !== void 0 && (this.l = [...t.l]), this.elementProperties = new Map(t.elementProperties);
  }
  static finalize() {
    if (this.hasOwnProperty(H("finalized")))
      return;
    if (this.finalized = !0, this._$Ei(), this.hasOwnProperty(H("properties"))) {
      const e = this.properties, r = [...Ft(e), ...Lt(e)];
      for (const s of r)
        this.createProperty(s, e[s]);
    }
    const t = this[Symbol.metadata];
    if (t !== null) {
      const e = litPropertyMetadata.get(t);
      if (e !== void 0)
        for (const [r, s] of e)
          this.elementProperties.set(r, s);
    }
    this._$Eh = /* @__PURE__ */ new Map();
    for (const [e, r] of this.elementProperties) {
      const s = this._$Eu(e, r);
      s !== void 0 && this._$Eh.set(s, e);
    }
    this.elementStyles = this.finalizeStyles(this.styles);
  }
  static finalizeStyles(t) {
    const e = [];
    if (Array.isArray(t)) {
      const r = new Set(t.flat(1 / 0).reverse());
      for (const s of r)
        e.unshift(ct(s));
    } else
      t !== void 0 && e.push(ct(t));
    return e;
  }
  static _$Eu(t, e) {
    const r = e.attribute;
    return r === !1 ? void 0 : typeof r == "string" ? r : typeof t == "string" ? t.toLowerCase() : void 0;
  }
  constructor() {
    super(), this._$Ep = void 0, this.isUpdatePending = !1, this.hasUpdated = !1, this._$Em = null, this._$Ev();
  }
  _$Ev() {
    var t;
    this._$Eg = new Promise((e) => this.enableUpdating = e), this._$AL = /* @__PURE__ */ new Map(), this._$E_(), this.requestUpdate(), (t = this.constructor.l) == null || t.forEach((e) => e(this));
  }
  addController(t) {
    var e;
    (this._$ES ?? (this._$ES = [])).push(t), this.renderRoot !== void 0 && this.isConnected && ((e = t.hostConnected) == null || e.call(t));
  }
  removeController(t) {
    var e;
    (e = this._$ES) == null || e.splice(this._$ES.indexOf(t) >>> 0, 1);
  }
  _$E_() {
    const t = /* @__PURE__ */ new Map(), e = this.constructor.elementProperties;
    for (const r of e.keys())
      this.hasOwnProperty(r) && (t.set(r, this[r]), delete this[r]);
    t.size > 0 && (this._$Ep = t);
  }
  createRenderRoot() {
    const t = this.shadowRoot ?? this.attachShadow(this.constructor.shadowRootOptions);
    return Pt(t, this.constructor.elementStyles), t;
  }
  connectedCallback() {
    var t;
    this.renderRoot ?? (this.renderRoot = this.createRenderRoot()), this.enableUpdating(!0), (t = this._$ES) == null || t.forEach((e) => {
      var r;
      return (r = e.hostConnected) == null ? void 0 : r.call(e);
    });
  }
  enableUpdating(t) {
  }
  disconnectedCallback() {
    var t;
    (t = this._$ES) == null || t.forEach((e) => {
      var r;
      return (r = e.hostDisconnected) == null ? void 0 : r.call(e);
    });
  }
  attributeChangedCallback(t, e, r) {
    this._$AK(t, r);
  }
  _$EO(t, e) {
    var i;
    const r = this.constructor.elementProperties.get(t), s = this.constructor._$Eu(t, r);
    if (s !== void 0 && r.reflect === !0) {
      const o = (((i = r.converter) == null ? void 0 : i.toAttribute) !== void 0 ? r.converter : J).toAttribute(e, r.type);
      this._$Em = t, o == null ? this.removeAttribute(s) : this.setAttribute(s, o), this._$Em = null;
    }
  }
  _$AK(t, e) {
    var i;
    const r = this.constructor, s = r._$Eh.get(t);
    if (s !== void 0 && this._$Em !== s) {
      const o = r.getPropertyOptions(s), l = typeof o.converter == "function" ? { fromAttribute: o.converter } : ((i = o.converter) == null ? void 0 : i.fromAttribute) !== void 0 ? o.converter : J;
      this._$Em = s, this[s] = l.fromAttribute(e, o.type), this._$Em = null;
    }
  }
  requestUpdate(t, e, r, s = !1, i) {
    if (t !== void 0) {
      if (r ?? (r = this.constructor.getPropertyOptions(t)), !(r.hasChanged ?? nt)(s ? i : this[t], e))
        return;
      this.C(t, e, r);
    }
    this.isUpdatePending === !1 && (this._$Eg = this._$EP());
  }
  C(t, e, r) {
    this._$AL.has(t) || this._$AL.set(t, e), r.reflect === !0 && this._$Em !== t && (this._$Ej ?? (this._$Ej = /* @__PURE__ */ new Set())).add(t);
  }
  async _$EP() {
    this.isUpdatePending = !0;
    try {
      await this._$Eg;
    } catch (e) {
      Promise.reject(e);
    }
    const t = this.scheduleUpdate();
    return t != null && await t, !this.isUpdatePending;
  }
  scheduleUpdate() {
    return this.performUpdate();
  }
  performUpdate() {
    var r;
    if (!this.isUpdatePending)
      return;
    if (!this.hasUpdated) {
      if (this._$Ep) {
        for (const [i, o] of this._$Ep)
          this[i] = o;
        this._$Ep = void 0;
      }
      const s = this.constructor.elementProperties;
      if (s.size > 0)
        for (const [i, o] of s)
          o.wrapped !== !0 || this._$AL.has(i) || this[i] === void 0 || this.C(i, this[i], o);
    }
    let t = !1;
    const e = this._$AL;
    try {
      t = this.shouldUpdate(e), t ? (this.willUpdate(e), (r = this._$ES) == null || r.forEach((s) => {
        var i;
        return (i = s.hostUpdate) == null ? void 0 : i.call(s);
      }), this.update(e)) : this._$ET();
    } catch (s) {
      throw t = !1, this._$ET(), s;
    }
    t && this._$AE(e);
  }
  willUpdate(t) {
  }
  _$AE(t) {
    var e;
    (e = this._$ES) == null || e.forEach((r) => {
      var s;
      return (s = r.hostUpdated) == null ? void 0 : s.call(r);
    }), this.hasUpdated || (this.hasUpdated = !0, this.firstUpdated(t)), this.updated(t);
  }
  _$ET() {
    this._$AL = /* @__PURE__ */ new Map(), this.isUpdatePending = !1;
  }
  get updateComplete() {
    return this.getUpdateComplete();
  }
  getUpdateComplete() {
    return this._$Eg;
  }
  shouldUpdate(t) {
    return !0;
  }
  update(t) {
    this._$Ej && (this._$Ej = this._$Ej.forEach((e) => this._$EO(e, this[e]))), this._$ET();
  }
  updated(t) {
  }
  firstUpdated(t) {
  }
}
L.elementStyles = [], L.shadowRootOptions = { mode: "open" }, L[H("elementProperties")] = /* @__PURE__ */ new Map(), L[H("finalized")] = /* @__PURE__ */ new Map(), z == null || z({ ReactiveElement: L }), (b.reactiveElementVersions ?? (b.reactiveElementVersions = [])).push("2.0.1");
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
const Kt = { attribute: !0, type: String, converter: J, reflect: !1, hasChanged: nt }, qt = (n = Kt, t, e) => {
  const { kind: r, metadata: s } = e;
  let i = globalThis.litPropertyMetadata.get(s);
  if (i === void 0 && globalThis.litPropertyMetadata.set(s, i = /* @__PURE__ */ new Map()), i.set(e.name, n), r === "accessor") {
    const { name: o } = e;
    return { set(l) {
      const a = t.get.call(this);
      t.set.call(this, l), this.requestUpdate(o, a, n);
    }, init(l) {
      return l !== void 0 && this.C(o, void 0, n), l;
    } };
  }
  if (r === "setter") {
    const { name: o } = e;
    return function(l) {
      const a = this[o];
      t.call(this, l), this.requestUpdate(o, a, n);
    };
  }
  throw Error("Unsupported decorator location: " + r);
};
function h(n) {
  return (t, e) => typeof e == "object" ? qt(n, t, e) : ((r, s, i) => {
    const o = s.hasOwnProperty(i);
    return s.constructor.createProperty(i, o ? { ...r, wrapped: !0 } : r), o ? Object.getOwnPropertyDescriptor(s, i) : void 0;
  })(n, t, e);
}
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
const et = (n, t, e) => (e.configurable = !0, e.enumerable = !0, Reflect.decorate && typeof t != "object" && Object.defineProperty(n, t, e), e);
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
function Ut(n, t) {
  return (e, r, s) => {
    const i = (o) => {
      var l;
      return ((l = o.renderRoot) == null ? void 0 : l.querySelector(n)) ?? null;
    };
    if (t) {
      const { get: o, set: l } = typeof r == "object" ? e : s ?? (() => {
        const a = Symbol();
        return { get() {
          return this[a];
        }, set(d) {
          this[a] = d;
        } };
      })();
      return et(e, r, { get() {
        if (t) {
          let a = o.call(this);
          return a === void 0 && (a = i(this), l.call(this, a)), a;
        }
        return i(this);
      } });
    }
    return et(e, r, { get() {
      return i(this);
    } });
  };
}
/**
 * @license
 * Copyright 2021 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
function ot(n) {
  return (t, e) => {
    const { slot: r, selector: s } = n ?? {}, i = "slot" + (r ? `[name=${r}]` : ":not([name])");
    return et(t, e, { get() {
      var a;
      const o = (a = this.renderRoot) == null ? void 0 : a.querySelector(i), l = (o == null ? void 0 : o.assignedElements(n)) ?? [];
      return s === void 0 ? l : l.filter((d) => d.matches(s));
    } });
  };
}
var Wt = Object.defineProperty, Zt = Object.getOwnPropertyDescriptor, Jt = (n, t, e, r) => {
  for (var s = r > 1 ? void 0 : r ? Zt(t, e) : t, i = n.length - 1, o; i >= 0; i--)
    (o = n[i]) && (s = (r ? o(t, e, s) : o(s)) || s);
  return r && s && Wt(t, e, s), s;
};
const At = (n) => {
  class t extends n {
    constructor() {
      super(...arguments), this.rounded = !0;
    }
  }
  return t.styles = [
    n.styles ?? [],
    M`
        :host > * {
          border-radius: 0;
        }
        :host([rounded]) > * {
          border-radius: 4px;
        }
      `
  ], Jt([
    h({ type: Boolean, reflect: !0 })
  ], t.prototype, "rounded", 2), t;
}, Nt = At(rt);
var Qt = Object.defineProperty, Gt = Object.getOwnPropertyDescriptor, Tt = (n, t, e, r) => {
  for (var s = r > 1 ? void 0 : r ? Gt(t, e) : t, i = n.length - 1, o; i >= 0; i--)
    (o = n[i]) && (s = (r ? o(t, e, s) : o(s)) || s);
  return r && s && Qt(t, e, s), s;
};
const bt = (n) => {
  class t extends n {
    constructor() {
      super(...arguments), this.condensed = !1;
    }
  }
  return t.styles = [
    n.styles ?? [],
    //TODO extract string to global file TOKENS
    M`
        :host,
        :host * {
          font-family: var(--type-family-regular);
        }
        :host([condensed]),
        :host([condensed]) * {
          font-family: var(--type-family-condensed);
        }
      `
  ], Tt([
    h({ type: Boolean, reflect: !0 })
  ], t.prototype, "condensed", 2), t;
};
bt(rt);
const O = At(bt(rt)), jt = M`.container{display:flex;display:inline-flex;align-items:center;background:var(--surface-accent-subtle);white-space:nowrap;line-height:20px}.container>.number{display:flex;width:28px;height:28px;padding:0;justify-content:center;align-items:center;text-align:center;font-size:14px;font-style:normal;font-weight:400;background:var(--surface-accent);color:var(--text-on-accent)}.container>.text{color:var(--text-default);padding:4px 8px;font-size:14px;font-style:normal;font-weight:400;height:20px}:host([rounded])>.container,:host([rounded])>.container>.number{border-radius:100px}
`;
var Xt = Object.defineProperty, Yt = Object.getOwnPropertyDescriptor, zt = Object.getPrototypeOf, $t = Reflect.get, it = (n, t, e, r) => {
  for (var s = r > 1 ? void 0 : r ? Yt(t, e) : t, i = n.length - 1, o; i >= 0; i--)
    (o = n[i]) && (s = (r ? o(t, e, s) : o(s)) || s);
  return r && s && Xt(t, e, s), s;
}, _t = (n, t, e) => $t(zt(n), e, t);
let w = class extends O {
  render() {
    return v`
      <div class="container">
        <div class="number">${this.number}</div>
        <div class="text">${this.text}</div>
      </div>
    `;
  }
};
w.styles = [jt, _t(w, w, "styles") ?? []];
it([
  h({ type: String })
], w.prototype, "text", 2);
it([
  h({ type: String || Number })
], w.prototype, "number", 2);
w = it([
  k("zeta-priority-pill")
], w);
const te = M`.container{display:inline-flex;justify-content:center;align-items:center;gap:8px;border-radius:2px;padding-top:1px;padding-bottom:1px;border:1px solid}.container>.icon-container{padding-left:7px}.container>.icon-container,.container>.icon-container div{display:inline-flex;justify-content:center;align-items:center}.container>.text{color:var(--text-default);font-size:14px;font-style:normal;font-weight:400;height:20px;line-height:20px;padding-right:7px}::slotted([slot=icon]){width:20px;height:20px}:host([rounded])>.container{border-radius:100px}.container,:host([status=neutral])>.container,:host([status=neutral])>.container svg{fill:var(--icon-subtle);border-color:var(--border-default-subtle);background:var(--surface-default-subtle)}:host([status=info])>.container,:host([status=info])>.container svg{border-color:var(--border-info-subtle);background:var(--surface-info-subtle);fill:var(--icon-info)}:host([status=positive])>.container,:host([status=positive])>.container svg{border-color:var(--border-positive-subtle);background:var(--surface-positive-subtle);fill:var(--icon-positive)}:host([status=warning])>.container,:host([status=warning])>.container svg{border-color:var(--border-warning-subtle);background:var(--surface-warning-subtle);fill:var(--icon-warning)}:host([status=negative])>.container,:host([status=negative])>.container svg{border-color:var(--border-negative-subtle);background:var(--surface-negative-subtle);fill:var(--icon-negative)}
`;
var ee = Object.defineProperty, re = Object.getOwnPropertyDescriptor, se = Object.getPrototypeOf, ne = Reflect.get, G = (n, t, e, r) => {
  for (var s = r > 1 ? void 0 : r ? re(t, e) : t, i = n.length - 1, o; i >= 0; i--)
    (o = n[i]) && (s = (r ? o(t, e, s) : o(s)) || s);
  return r && s && ee(t, e, s), s;
}, oe = (n, t, e) => ne(se(n), e, t);
let x = class extends O {
  constructor() {
    super(...arguments), this.status = "neutral";
  }
  render() {
    const n = Dt`
    <svg xmlns="http://www.w3.org/2000/svg" width="8" height="20" viewBox="0 0 8 8" >
    <circle cx="4" cy="4" r="4" />
    </svg>`;
    return v`
      <div class="container">
        <div class="icon-container">
          ${this.hasIcon == null || this.hasIcon.length == 0 ? n : tt}
          <slot name="icon" @slotchange=${() => this.requestUpdate()}></slot>
        </div>
        <div class="text">${this.text ? this.text : v`<slot class="text" name="text"></slot>`}</div>
      </div>
    `;
  }
};
x.styles = [oe(x, x, "styles") ?? [], te];
G([
  h({ type: String, reflect: !0 })
], x.prototype, "status", 2);
G([
  h({ type: String })
], x.prototype, "text", 2);
G([
  ot({ slot: "icon" })
], x.prototype, "hasIcon", 2);
x = G([
  k("zeta-status-label")
], x);
const ie = M`.banner{display:flex;flex-direction:row;padding:12px;justify-content:center;align-items:center;border:1px solid;color:var(--text-default)}.banner>.leading{padding-right:8px;padding-top:2px;align-self:stretch}.banner>.trailing{flex:1}.banner>.trailing>.header{display:flex;flex-direction:row;justify-content:space-between}.banner>.trailing>.header svg#close{fill:var(--icon-default);cursor:pointer}.banner>.trailing>.header>.title{font-size:16px;font-style:normal;font-weight:500;line-height:24px}.banner>.trailing>.body{margin-top:4px;font-size:14px;font-style:normal;font-weight:400;line-height:20px}.banner>.trailing>.footer{display:flex;flex-direction:row;gap:8px}:host([rounded])>.banner{border-radius:4px}.banner,:host([status=default])>.banner{border-color:var(--border-default);background:var(--surface-default-subtle);fill:var(--icon-default)}:host([status=info])>.banner{border-color:var(--border-info);background:var(--surface-info-subtle);fill:var(--icon-info)}:host([status=positive])>.banner{border-color:var(--border-positive);background:var(--surface-positive-subtle);fill:var(--icon-positive)}:host([status=warning])>.banner{border-color:var(--border-warning);background:var(--surface-warning-subtle);fill:var(--icon-warning)}:host([status=negative])>.banner{border-color:var(--border-negative);background:var(--surface-negative-subtle);fill:var(--icon-negative)}::slotted([slot=leading-action]),::slotted([slot=trailing-action]){margin-top:16px}
`, ae = M`:host{width:fit-content;height:fit-content}:host>button{cursor:pointer;display:flex;align-items:center;justify-content:center;overflow-x:ellipsis;font-size:16px;font-style:normal;font-weight:500;line-height:24px;border:none}:host([size=large])>button{padding:12px 24px}:host>button,:host([size=medium])>button{padding:8px 16px}:host([size=small])>button{padding:6px 12px;font-size:14px;line-height:20px}:host([size=large][condensed])>button{padding:inherit 16px}:host([size=medium][condensed])>button{padding:inherit 12px}:host([size=small][condensed])>button{padding:inherit 8px}:host(:focus)>button{outline:none;box-shadow:0 0 0 2px var(--color-border-focused)}:host>button,:host([flavor=primary])>button{background-color:var(--action-primary);color:var(--text-on-action-primary)}:host>button:hover:not([disabled]),:host([flavor=primary])>button:hover:not([disabled]){background-color:var(--action-primary-hovered)}:host>button:active:not([disabled]),:host([flavor=primary])>button:active:not([disabled]){background-color:var(--action-primary-pressed)}:host>button:focus:not([disabled]),:host([flavor=primary])>button:focus:not([disabled]){background-color:var(--action-primary-focused)}:host([flavor=primary-variant])>button{background-color:var(--action-primary-variant);color:var(--text-on-primary-variant)}:host([flavor=primary-variant])>button:hover:not([disabled]){background-color:var(--action-primary-variant-hovered)}:host([flavor=primary-variant])>button:active:not([disabled]){background-color:var(--action-primary-variant-pressed)}:host([flavor=primary-variant])>button:focus:not([disabled]){background-color:var(--action-primary-variant-focused)}:host([flavor=negative])>button{background-color:var(--action-negative);color:var(--text-on-action-negative)}:host([flavor=negative])>button:hover:not([disabled]){background-color:var(--action-negative-hovered)}:host([flavor=negative])>button:active:not([disabled]){background-color:var(--action-negative-pressed)}:host([flavor=negative])>button:focus:not([disabled]){background-color:var(--action-negative-focused)}:host([flavor=outline])>button{background-color:var(--action-outline);color:var(--text-accent);border:1px solid var(--action-primary-outline-border)}:host([flavor=outline])>button:hover:not([disabled]){background-color:var(--action-outline-hovered)}:host([flavor=outline])>button:active:not([disabled]){background-color:var(--action-outline-pressed)}:host([flavor=outline])>button:focus:not([disabled]){background-color:var(--action-outline-focused)}:host([disabled])>button{cursor:not-allowed;background:var(--action-disabled);color:var(--text-on-action-disabled)}
`;
var le = Object.defineProperty, ce = Object.getOwnPropertyDescriptor, C = (n, t, e, r) => {
  for (var s = r > 1 ? void 0 : r ? ce(t, e) : t, i = n.length - 1, o; i >= 0; i--)
    (o = n[i]) && (s = (r ? o(t, e, s) : o(s)) || s);
  return r && s && le(t, e, s), s;
};
class at extends O {
}
at.shadowRootOptions = {
  mode: "open",
  delegatesFocus: !0
};
at.styles = [ae, O.styles || []];
let f = class extends at {
  constructor() {
    super(...arguments), this.disabled = !1, this.name = "", this.value = "", this.flavor = "primary", this.size = "medium";
  }
  focus() {
    var n;
    (n = this.buttonElement) == null || n.focus();
  }
  blur() {
    var n;
    (n = this.buttonElement) == null || n.blur();
  }
  static get styles() {
    return [super.styles ?? []];
  }
  render() {
    return v`
      <button ?disabled=${this.disabled} value=${this.value} name=${this.name}>
        <slot></slot>
      </button>
    `;
  }
};
f.shadowRootOptions = {
  mode: "open",
  delegatesFocus: !0
};
C([
  Ut("button")
], f.prototype, "buttonElement", 2);
C([
  h({ type: Boolean, reflect: !0 })
], f.prototype, "disabled", 2);
C([
  h({ type: String })
], f.prototype, "name", 2);
C([
  h({ type: String })
], f.prototype, "value", 2);
C([
  h({ type: String, reflect: !0 })
], f.prototype, "flavor", 2);
C([
  h({ type: String, reflect: !0 })
], f.prototype, "size", 2);
f = C([
  k("zeta-button")
], f);
var de = Object.defineProperty, he = Object.getOwnPropertyDescriptor, pe = Object.getPrototypeOf, ue = Reflect.get, F = (n, t, e, r) => {
  for (var s = r > 1 ? void 0 : r ? he(t, e) : t, i = n.length - 1, o; i >= 0; i--)
    (o = n[i]) && (s = (r ? o(t, e, s) : o(s)) || s);
  return r && s && de(t, e, s), s;
}, ve = (n, t, e) => ue(pe(n), e, t);
let g = class extends O {
  constructor() {
    super(...arguments), this.title = "", this.body = "", this.status = "default", this.getIcon = () => {
      switch (this.status) {
        case "positive":
          return v`<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20">
          <g clip-path="url(#clip0_1016_17892)">
            <path
              d="M9.99935 1.66663C5.39935 1.66663 1.66602 5.39996 1.66602 9.99996C1.66602 14.6 5.39935 18.3333 9.99935 18.3333C14.5993 18.3333 18.3327 14.6 18.3327 9.99996C18.3327 5.39996 14.5993 1.66663 9.99935 1.66663ZM7.74102 13.575L4.74935 10.5833C4.42435 10.2583 4.42435 9.73329 4.74935 9.40829C5.07435 9.08329 5.59935 9.08329 5.92435 9.40829L8.33268 11.8083L14.066 6.07496C14.391 5.74996 14.916 5.74996 15.241 6.07496C15.566 6.39996 15.566 6.92496 15.241 7.24996L8.91602 13.575C8.59935 13.9 8.06602 13.9 7.74102 13.575Z"
            />
          </g>
        </svg>`;
        case "warning":
          return v`<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20">
          <g clip-path="url(#clip0_1016_18541)">
            <path
              d="M3.72445 17.5001H16.2745C17.5578 17.5001 18.3578 16.1084 17.7161 15.0001L11.4411 4.1584C10.7994 3.05007 9.19945 3.05007 8.55778 4.1584L2.28278 15.0001C1.64112 16.1084 2.44112 17.5001 3.72445 17.5001ZM9.99945 11.6667C9.54112 11.6667 9.16612 11.2917 9.16612 10.8334V9.16673C9.16612 8.7084 9.54112 8.3334 9.99945 8.3334C10.4578 8.3334 10.8328 8.7084 10.8328 9.16673V10.8334C10.8328 11.2917 10.4578 11.6667 9.99945 11.6667ZM10.8328 15.0001H9.16612V13.3334H10.8328V15.0001Z"
            />
          </g>
        </svg>`;
        case "negative":
          return v`<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20">
          <g clip-path="url(#clip0_1016_19150)">
            <path
              d="M9.99935 1.66675C5.39935 1.66675 1.66602 5.40008 1.66602 10.0001C1.66602 14.6001 5.39935 18.3334 9.99935 18.3334C14.5993 18.3334 18.3327 14.6001 18.3327 10.0001C18.3327 5.40008 14.5993 1.66675 9.99935 1.66675ZM9.99935 10.8334C9.54102 10.8334 9.16602 10.4584 9.16602 10.0001V6.66675C9.16602 6.20842 9.54102 5.83342 9.99935 5.83342C10.4577 5.83342 10.8327 6.20842 10.8327 6.66675V10.0001C10.8327 10.4584 10.4577 10.8334 9.99935 10.8334ZM10.8327 14.1667H9.16602V12.5001H10.8327V14.1667Z"
            />
          </g>
        </svg>`;
        case "default":
        case "info":
        default:
          return v`<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20">
          <g clip-path="url(#clip0_1016_19763)">
            <path
              d="M9.99935 1.66669C5.39935 1.66669 1.66602 5.40002 1.66602 10C1.66602 14.6 5.39935 18.3334 9.99935 18.3334C14.5993 18.3334 18.3327 14.6 18.3327 10C18.3327 5.40002 14.5993 1.66669 9.99935 1.66669ZM9.99935 14.1667C9.54102 14.1667 9.16602 13.7917 9.16602 13.3334V10C9.16602 9.54169 9.54102 9.16669 9.99935 9.16669C10.4577 9.16669 10.8327 9.54169 10.8327 10V13.3334C10.8327 13.7917 10.4577 14.1667 9.99935 14.1667ZM10.8327 7.50002H9.16602V5.83335H10.8327V7.50002Z"
            />
          </g>
        </svg>`;
      }
    }, this.getCloseIcon = () => v`<svg id="close" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" .onclick=${() => this.remove()}>
      <g clip-path="url(#clip0_1016_14731)">
        <path
          d="M15.2496 4.75834C14.9246 4.43334 14.3996 4.43334 14.0746 4.75834L9.99961 8.82501L5.92461 4.75001C5.59961 4.42501 5.07461 4.42501 4.74961 4.75001C4.42461 5.07501 4.42461 5.60001 4.74961 5.92501L8.82461 10L4.74961 14.075C4.42461 14.4 4.42461 14.925 4.74961 15.25C5.07461 15.575 5.59961 15.575 5.92461 15.25L9.99961 11.175L14.0746 15.25C14.3996 15.575 14.9246 15.575 15.2496 15.25C15.5746 14.925 15.5746 14.4 15.2496 14.075L11.1746 10L15.2496 5.92501C15.5663 5.60834 15.5663 5.07501 15.2496 4.75834Z"
        />
      </g>
    </svg>`, this.styleButtons = () => {
      if (this.requestUpdate(), this.leadingAction[0] && this.leadingAction[0] instanceof f) {
        const n = this.leadingAction[0];
        n.flavor = "outline-subtle", n.rounded = this.rounded, n.condensed = this.condensed;
      }
      if (this.trailingAction[0] && this.trailingAction[0] instanceof f) {
        const n = this.trailingAction[0];
        n.flavor = "outline-subtle", n.rounded = this.rounded, n.condensed = this.condensed;
      }
    };
  }
  render() {
    return this.styleButtons(), v`
      <div class="banner">
        <div class="leading">${this.getIcon()}</div>
        <div class="trailing">
          <div class="header">
            <div class="title">${this.title}</div>
            <div>${this.getCloseIcon()}</div>
          </div>
          <div class="body">${this.body}</div>
          <div class="footer ">
            <slot name="leading-action" @slotchange=${this.styleButtons}></slot>
            <slot name="trailing-action" @slotchange=${this.styleButtons}></slot>
          </div>
        </div>
      </div>
    `;
  }
};
g.styles = [ie, ve(g, g, "styles") ?? []];
F([
  h({ type: String })
], g.prototype, "title", 2);
F([
  h({ type: String })
], g.prototype, "body", 2);
F([
  h({ type: String, reflect: !0 })
], g.prototype, "status", 2);
F([
  ot({ slot: "leading-action", flatten: !0 })
], g.prototype, "leadingAction", 2);
F([
  ot({ slot: "trailing-action", flatten: !0 })
], g.prototype, "trailingAction", 2);
g = F([
  k("zeta-in-page-banner")
], g);
const fe = M`.system-banner{height:24px;line-height:24px;white-space:nowrap;padding:8px 16px;display:flex;justify-content:space-between;overflow:hidden}.system-banner>div{display:flex}.system-banner>.text,.system-banner>.icon{font-size:16px;text-align:"center";font-style:normal;font-weight:500;line-height:24px}.system-banner .leading,.system-banner ::slotted([slot="leading icon"]){padding-inline-end:8px}.system-banner .trailing,.system-banner ::slotted([slot="trailing icon"]){padding-inline-start:8px;justify-self:end}.system-banner,:host([status=default])>.system-banner{background:var(--surface-accent);color:var(--text-on-accent);fill:var(--text-on-accent)}:host([status=positive])>.system-banner{background:var(--surface-positive);color:var(--text-on-positive);fill:var(--text-on-positive)}:host([status=warning])>.system-banner{background:var(--surface-warning);color:var(--text-on-warning);fill:var(--text-on-warning)}:host([status=negative])>.system-banner{background:var(--surface-negative);color:var(--text-on-negative);fill:var(--text-on-negative)}:host([rounded])>.system-banner{border-radius:0!important}
`;
var ge = Object.defineProperty, me = Object.getOwnPropertyDescriptor, ye = Object.getPrototypeOf, Ae = Reflect.get, T = (n, t, e, r) => {
  for (var s = r > 1 ? void 0 : r ? me(t, e) : t, i = n.length - 1, o; i >= 0; i--)
    (o = n[i]) && (s = (r ? o(t, e, s) : o(s)) || s);
  return r && s && ge(t, e, s), s;
}, be = (n, t, e) => Ae(ye(n), e, t);
let S = class extends O {
  constructor() {
    super(...arguments), this.status = "default", this.align = "start";
  }
  render() {
    const n = v`<slot name="leading icon" class="leading icon"> </slot>`, t = this.text ? v`<div class="text">${this.text}</div>` : v`<slot name="text"></slot>`;
    return v`
      <div class="system-banner">
        <div>${this.align == "start" ? [n, t] : tt}</div>
        <div>${this.align != "start" ? [n, t] : tt}</div>
        <div><slot name="trailing icon" class="trailing icon"></slot></div>
      </div>
    `;
  }
};
S.styles = [be(S, S, "styles") || [], fe];
T([
  h({ type: String, reflect: !0 })
], S.prototype, "status", 2);
T([
  h({ type: String, reflect: !0 })
], S.prototype, "align", 2);
T([
  h({ type: String })
], S.prototype, "text", 2);
S = T([
  k("zeta-system-banner")
], S);
const xe = M`:host .icon{flex-shrink:0;max-width:initial;font-weight:400;font-style:normal;display:inline-block;line-height:1;text-transform:none;letter-spacing:normal;word-wrap:normal;white-space:nowrap;direction:ltr;-webkit-font-smoothing:antialiased;text-rendering:optimizeLegibility;-moz-osx-font-smoothing:grayscale;font-feature-settings:"liga";-webkit-user-select:none;user-select:none}:host .icon.rounded{font-family:zeta-icons-round!important}@font-face{font-family:zeta-icons-round;src:url(data:font/woff2;base64,) format("woff2")}:host .icon.sharp{font-family:zeta-icons-sharp!important}@font-face{font-family:zeta-icons-sharp;src:url(data:font/woff2;base64,) format("woff2")}
`;
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
const K = globalThis, N = K.trustedTypes, pt = N ? N.createPolicy("lit-html", { createHTML: (n) => n }) : void 0, xt = "$lit$", A = `lit$${(Math.random() + "").slice(9)}$`, St = "?" + A, Se = `<${St}>`, D = document, Q = () => D.createComment(""), q = (n) => n === null || typeof n != "object" && typeof n != "function", Mt = Array.isArray, Me = (n) => Mt(n) || typeof (n == null ? void 0 : n[Symbol.iterator]) == "function", $ = `[ 	
\f\r]`, R = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, ut = /-->/g, vt = />/g, V = RegExp(`>|${$}(?:([^\\s"'>=/]+)(${$}*=${$}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`, "g"), ft = /'/g, gt = /"/g, Vt = /^(?:script|style|textarea|title)$/i, E = Symbol.for("lit-noChange"), p = Symbol.for("lit-nothing"), mt = /* @__PURE__ */ new WeakMap(), B = D.createTreeWalker(D, 129);
function Bt(n, t) {
  if (!Array.isArray(n) || !n.hasOwnProperty("raw"))
    throw Error("invalid template strings array");
  return pt !== void 0 ? pt.createHTML(t) : t;
}
const Ve = (n, t) => {
  const e = n.length - 1, r = [];
  let s, i = t === 2 ? "<svg>" : "", o = R;
  for (let l = 0; l < e; l++) {
    const a = n[l];
    let d, u, c = -1, m = 0;
    for (; m < a.length && (o.lastIndex = m, u = o.exec(a), u !== null); )
      m = o.lastIndex, o === R ? u[1] === "!--" ? o = ut : u[1] !== void 0 ? o = vt : u[2] !== void 0 ? (Vt.test(u[2]) && (s = RegExp("</" + u[2], "g")), o = V) : u[3] !== void 0 && (o = V) : o === V ? u[0] === ">" ? (o = s ?? R, c = -1) : u[1] === void 0 ? c = -2 : (c = o.lastIndex - u[2].length, d = u[1], o = u[3] === void 0 ? V : u[3] === '"' ? gt : ft) : o === gt || o === ft ? o = V : o === ut || o === vt ? o = R : (o = V, s = void 0);
    const y = o === V && n[l + 1].startsWith("/>") ? " " : "";
    i += o === R ? a + Se : c >= 0 ? (r.push(d), a.slice(0, c) + xt + a.slice(c) + A + y) : a + A + (c === -2 ? l : y);
  }
  return [Bt(n, i + (n[e] || "<?>") + (t === 2 ? "</svg>" : "")), r];
};
class U {
  constructor({ strings: t, _$litType$: e }, r) {
    let s;
    this.parts = [];
    let i = 0, o = 0;
    const l = t.length - 1, a = this.parts, [d, u] = Ve(t, e);
    if (this.el = U.createElement(d, r), B.currentNode = this.el.content, e === 2) {
      const c = this.el.content.firstChild;
      c.replaceWith(...c.childNodes);
    }
    for (; (s = B.nextNode()) !== null && a.length < l; ) {
      if (s.nodeType === 1) {
        if (s.hasAttributes())
          for (const c of s.getAttributeNames())
            if (c.endsWith(xt)) {
              const m = u[o++], y = s.getAttribute(c).split(A), W = /([.?@])?(.*)/.exec(m);
              a.push({ type: 1, index: i, name: W[2], strings: y, ctor: W[1] === "." ? we : W[1] === "?" ? De : W[1] === "@" ? Ce : X }), s.removeAttribute(c);
            } else
              c.startsWith(A) && (a.push({ type: 6, index: i }), s.removeAttribute(c));
        if (Vt.test(s.tagName)) {
          const c = s.textContent.split(A), m = c.length - 1;
          if (m > 0) {
            s.textContent = N ? N.emptyScript : "";
            for (let y = 0; y < m; y++)
              s.append(c[y], Q()), B.nextNode(), a.push({ type: 2, index: ++i });
            s.append(c[m], Q());
          }
        }
      } else if (s.nodeType === 8)
        if (s.data === St)
          a.push({ type: 2, index: i });
        else {
          let c = -1;
          for (; (c = s.data.indexOf(A, c + 1)) !== -1; )
            a.push({ type: 7, index: i }), c += A.length - 1;
        }
      i++;
    }
  }
  static createElement(t, e) {
    const r = D.createElement("template");
    return r.innerHTML = t, r;
  }
}
function P(n, t, e = n, r) {
  var o, l;
  if (t === E)
    return t;
  let s = r !== void 0 ? (o = e._$Co) == null ? void 0 : o[r] : e._$Cl;
  const i = q(t) ? void 0 : t._$litDirective$;
  return (s == null ? void 0 : s.constructor) !== i && ((l = s == null ? void 0 : s._$AO) == null || l.call(s, !1), i === void 0 ? s = void 0 : (s = new i(n), s._$AT(n, e, r)), r !== void 0 ? (e._$Co ?? (e._$Co = []))[r] = s : e._$Cl = s), s !== void 0 && (t = P(n, s._$AS(n, t.values), s, r)), t;
}
class Be {
  constructor(t, e) {
    this._$AV = [], this._$AN = void 0, this._$AD = t, this._$AM = e;
  }
  get parentNode() {
    return this._$AM.parentNode;
  }
  get _$AU() {
    return this._$AM._$AU;
  }
  u(t) {
    const { el: { content: e }, parts: r } = this._$AD, s = ((t == null ? void 0 : t.creationScope) ?? D).importNode(e, !0);
    B.currentNode = s;
    let i = B.nextNode(), o = 0, l = 0, a = r[0];
    for (; a !== void 0; ) {
      if (o === a.index) {
        let d;
        a.type === 2 ? d = new j(i, i.nextSibling, this, t) : a.type === 1 ? d = new a.ctor(i, a.name, a.strings, this, t) : a.type === 6 && (d = new Ee(i, this, t)), this._$AV.push(d), a = r[++l];
      }
      o !== (a == null ? void 0 : a.index) && (i = B.nextNode(), o++);
    }
    return B.currentNode = D, s;
  }
  p(t) {
    let e = 0;
    for (const r of this._$AV)
      r !== void 0 && (r.strings !== void 0 ? (r._$AI(t, r, e), e += r.strings.length - 2) : r._$AI(t[e])), e++;
  }
}
class j {
  get _$AU() {
    var t;
    return ((t = this._$AM) == null ? void 0 : t._$AU) ?? this._$Cv;
  }
  constructor(t, e, r, s) {
    this.type = 2, this._$AH = p, this._$AN = void 0, this._$AA = t, this._$AB = e, this._$AM = r, this.options = s, this._$Cv = (s == null ? void 0 : s.isConnected) ?? !0;
  }
  get parentNode() {
    let t = this._$AA.parentNode;
    const e = this._$AM;
    return e !== void 0 && (t == null ? void 0 : t.nodeType) === 11 && (t = e.parentNode), t;
  }
  get startNode() {
    return this._$AA;
  }
  get endNode() {
    return this._$AB;
  }
  _$AI(t, e = this) {
    t = P(this, t, e), q(t) ? t === p || t == null || t === "" ? (this._$AH !== p && this._$AR(), this._$AH = p) : t !== this._$AH && t !== E && this._(t) : t._$litType$ !== void 0 ? this.g(t) : t.nodeType !== void 0 ? this.$(t) : Me(t) ? this.T(t) : this._(t);
  }
  k(t) {
    return this._$AA.parentNode.insertBefore(t, this._$AB);
  }
  $(t) {
    this._$AH !== t && (this._$AR(), this._$AH = this.k(t));
  }
  _(t) {
    this._$AH !== p && q(this._$AH) ? this._$AA.nextSibling.data = t : this.$(D.createTextNode(t)), this._$AH = t;
  }
  g(t) {
    var i;
    const { values: e, _$litType$: r } = t, s = typeof r == "number" ? this._$AC(t) : (r.el === void 0 && (r.el = U.createElement(Bt(r.h, r.h[0]), this.options)), r);
    if (((i = this._$AH) == null ? void 0 : i._$AD) === s)
      this._$AH.p(e);
    else {
      const o = new Be(s, this), l = o.u(this.options);
      o.p(e), this.$(l), this._$AH = o;
    }
  }
  _$AC(t) {
    let e = mt.get(t.strings);
    return e === void 0 && mt.set(t.strings, e = new U(t)), e;
  }
  T(t) {
    Mt(this._$AH) || (this._$AH = [], this._$AR());
    const e = this._$AH;
    let r, s = 0;
    for (const i of t)
      s === e.length ? e.push(r = new j(this.k(Q()), this.k(Q()), this, this.options)) : r = e[s], r._$AI(i), s++;
    s < e.length && (this._$AR(r && r._$AB.nextSibling, s), e.length = s);
  }
  _$AR(t = this._$AA.nextSibling, e) {
    var r;
    for ((r = this._$AP) == null ? void 0 : r.call(this, !1, !0, e); t && t !== this._$AB; ) {
      const s = t.nextSibling;
      t.remove(), t = s;
    }
  }
  setConnected(t) {
    var e;
    this._$AM === void 0 && (this._$Cv = t, (e = this._$AP) == null || e.call(this, t));
  }
}
class X {
  get tagName() {
    return this.element.tagName;
  }
  get _$AU() {
    return this._$AM._$AU;
  }
  constructor(t, e, r, s, i) {
    this.type = 1, this._$AH = p, this._$AN = void 0, this.element = t, this.name = e, this._$AM = s, this.options = i, r.length > 2 || r[0] !== "" || r[1] !== "" ? (this._$AH = Array(r.length - 1).fill(new String()), this.strings = r) : this._$AH = p;
  }
  _$AI(t, e = this, r, s) {
    const i = this.strings;
    let o = !1;
    if (i === void 0)
      t = P(this, t, e, 0), o = !q(t) || t !== this._$AH && t !== E, o && (this._$AH = t);
    else {
      const l = t;
      let a, d;
      for (t = i[0], a = 0; a < i.length - 1; a++)
        d = P(this, l[r + a], e, a), d === E && (d = this._$AH[a]), o || (o = !q(d) || d !== this._$AH[a]), d === p ? t = p : t !== p && (t += (d ?? "") + i[a + 1]), this._$AH[a] = d;
    }
    o && !s && this.O(t);
  }
  O(t) {
    t === p ? this.element.removeAttribute(this.name) : this.element.setAttribute(this.name, t ?? "");
  }
}
class we extends X {
  constructor() {
    super(...arguments), this.type = 3;
  }
  O(t) {
    this.element[this.name] = t === p ? void 0 : t;
  }
}
class De extends X {
  constructor() {
    super(...arguments), this.type = 4;
  }
  O(t) {
    this.element.toggleAttribute(this.name, !!t && t !== p);
  }
}
class Ce extends X {
  constructor(t, e, r, s, i) {
    super(t, e, r, s, i), this.type = 5;
  }
  _$AI(t, e = this) {
    if ((t = P(this, t, e, 0) ?? p) === E)
      return;
    const r = this._$AH, s = t === p && r !== p || t.capture !== r.capture || t.once !== r.once || t.passive !== r.passive, i = t !== p && (r === p || s);
    s && this.element.removeEventListener(this.name, this, r), i && this.element.addEventListener(this.name, this, t), this._$AH = t;
  }
  handleEvent(t) {
    var e;
    typeof this._$AH == "function" ? this._$AH.call(((e = this.options) == null ? void 0 : e.host) ?? this.element, t) : this._$AH.handleEvent(t);
  }
}
class Ee {
  constructor(t, e, r) {
    this.element = t, this.type = 6, this._$AN = void 0, this._$AM = e, this.options = r;
  }
  get _$AU() {
    return this._$AM._$AU;
  }
  _$AI(t) {
    P(this, t);
  }
}
const _ = K.litHtmlPolyfillSupport;
_ == null || _(U, j), (K.litHtmlVersions ?? (K.litHtmlVersions = [])).push("3.0.2");
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
const Pe = { ATTRIBUTE: 1, CHILD: 2, PROPERTY: 3, BOOLEAN_ATTRIBUTE: 4, EVENT: 5, ELEMENT: 6 }, Ie = (n) => (...t) => ({ _$litDirective$: n, values: t });
let ke = class {
  constructor(t) {
  }
  get _$AU() {
    return this._$AM._$AU;
  }
  _$AT(t, e, r) {
    this._$Ct = t, this._$AM = e, this._$Ci = r;
  }
  _$AS(t, e) {
    return this.update(t, e);
  }
  update(t, e) {
    return this.render(...e);
  }
};
/**
 * @license
 * Copyright 2018 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
const wt = "important", Oe = " !" + wt, Fe = Ie(class extends ke {
  constructor(n) {
    var t;
    if (super(n), n.type !== Pe.ATTRIBUTE || n.name !== "style" || ((t = n.strings) == null ? void 0 : t.length) > 2)
      throw Error("The `styleMap` directive must be used in the `style` attribute and must be the only part in the attribute.");
  }
  render(n) {
    return Object.keys(n).reduce((t, e) => {
      const r = n[e];
      return r == null ? t : t + `${e = e.includes("-") ? e : e.replace(/(?:^(webkit|moz|ms|o)|)(?=[A-Z])/g, "-$&").toLowerCase()}:${r};`;
    }, "");
  }
  update(n, [t]) {
    const { style: e } = n.element;
    if (this.ut === void 0)
      return this.ut = new Set(Object.keys(t)), this.render(t);
    for (const r of this.ut)
      t[r] == null && (this.ut.delete(r), r.includes("-") ? e.removeProperty(r) : e[r] = null);
    for (const r in t) {
      const s = t[r];
      if (s != null) {
        this.ut.add(r);
        const i = typeof s == "string" && s.endsWith(Oe);
        r.includes("-") || i ? e.setProperty(r, i ? s.slice(0, -11) : s, i ? wt : "") : e[r] = s;
      }
    }
    return E;
  }
});
var Le = Object.defineProperty, Re = Object.getOwnPropertyDescriptor, Y = (n, t, e, r) => {
  for (var s = r > 1 ? void 0 : r ? Re(t, e) : t, i = n.length - 1, o; i >= 0; i--)
    (o = n[i]) && (s = (r ? o(t, e, s) : o(s)) || s);
  return r && s && Le(t, e, s), s;
};
let I = class extends Nt {
  constructor() {
    super(...arguments), this.name = void 0, this.size = 24, this.color = "black";
  }
  render() {
    const n = Fe({
      color: this.color,
      fontSize: typeof this.size == "number" ? this.size + "px" : this.size
    });
    return v` <span class="icon ${this.rounded ? "rounded" : "sharp"}" style=${n}>
      <slot @slotchange=${() => this.requestUpdate()}></slot>
      ${this.name}
    </span>`;
  }
};
I.styles = [xe];
Y([
  h({ type: String })
], I.prototype, "name", 2);
Y([
  h({ type: Number || String })
], I.prototype, "size", 2);
Y([
  h({ type: String })
], I.prototype, "color", 2);
I = Y([
  k("zeta-icon")
], I);
export {
  at as ButtonBase,
  f as ZetaButton,
  I as ZetaIcon,
  g as ZetaInPageBanner,
  w as ZetaPriorityPill,
  x as ZetaStatusLabel,
  S as ZetaSystemBanner
};
